...
| `GatherTime` | 文字列(HH:MM) | `09:30` | 集合時間 |
| `DismissTime` | 文字列(HH:MM) | `12:30` | 解散時間 |
...
| `Accompany` | 文字列 | `鈴木, 田中` | 付き添い（カンマ区切りの担当者名） |
...
| `ID` | 文字列 | `mb_...` | 主キー |
| `Name` | 文字列 | `井上 咲笑` | 氏名 |
| `Furigana` | 文字列 | `いのうえ さえ` | ふりがな（並び替え・検索に使用） |
...
| `Photos` | JSON文字列 | `[{"name":"実演.jpg","url":"https://..."}]` | 写真リスト（最大5枚） |
