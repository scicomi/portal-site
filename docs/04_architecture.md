...
- **Optimistic UI**: `saveOptimistic()` でローカルを即更新し、GAS保存は非同期で裏実行。失敗時はロールバック
...
ユーザーが「保存」クリック
  ↓
saveOptimistic()
  ├── ローカルリスト即更新 + キャッシュ書込 → UI即反映（_pending: true）
  └── api.save() → GAS → Sheets（非同期）
        ├── 成功 → ローカルリストをGAS応答で上書き → _pending 消去
        └── 失敗 → ロールバック → onRollback() でUI復元 → エラートースト
```
...
ユーザーが「削除」クリック
  ↓
UIから即除去 + 5秒トースト表示（「元に戻す」ボタン付き）
  ↓
5秒経過（UNDOなし）→ api.delete() → GAS → Sheets
  or
ユーザーが「元に戻す」クリック → キャンセル → UIに戻す
```
...
| パスワード漏洩 | トークンは localStorage 保存。パスワード自体はネットワーク上を1回だけ流れる |
